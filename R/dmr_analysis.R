...
dm_stat_test <- function(met_mat, cells_of_interest, bg_cells = NULL, test_type = 't', minimum_cell_count = 5)
{
  ...
  filtering = non_na_cell_counts_for_cells_of_interest > minimum_cell_count & non_na_cell_counts_for_bg_cells > minimum_cell_count
...
dm_stat_test_for_clusters <- function(dim_red_list)
{
  
  all_cells = colnames(dim_red_list$met_mat_for_features)
  cluster_ids = sort(unique(dim_red_list$clusters))
  dm_result_list = list()
  ...
  {
    cluster_cells = all_cells[dim_red_list$clusters == cluster_id]
    
    dm_result = dm_stat_test(met_mat = dim_red_list$met_mat_for_features,
